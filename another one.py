...
# Load data
df = pd.read_csv("crop ata.csv", on_bad_lines="skip")

# Features for crop selection and yield prediction
features = ["N_percent", "P_mg_per_kg", "K_cmol_per_kg", "pH", "Temp_C", "Rainfall_mm"]

# Clean data: Convert features to numeric and drop bad rows
for col in features:
    df[col] = pd.to_numeric(df[col], errors="coerce")
df = df.dropna(subset=features + ["Crop", "Yield_t_per_ha"])
...
plt.title("Yield Prediction Model RMSE Comparison")
plt.tight_layout()
plt.show()
