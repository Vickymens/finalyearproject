...
# Plot classification report heatmap for each model
from sklearn.metrics import classification_report

for name, model in fitted_classifiers.items():
    y_pred = model.predict(Xc_test)
    report = classification_report(
        yc_test, y_pred, target_names=le_crop.classes_, output_dict=True
    )
...
# --- After all model training and evaluation ---

# Save the best crop selection model
best_cls_row = max(cls_results, key=lambda x: x["Accuracy"])
best_cls_model = fitted_classifiers[best_cls_row["Model"]]
joblib.dump(best_cls_model, "testing_it_again_crop_selection_model.joblib")
joblib.dump(le_crop, "testing_it_again_crop_label_encoder.joblib")
joblib.dump(scaler_cls, "testing_it_again_cls_scaler.joblib")

# Save the best yield prediction model
best_reg_row = max(reg_results, key=lambda x: x["R2"])
best_reg_model = fitted_regressors[best_reg_row["Model"]]
joblib.dump(best_reg_model, "testing_it_again_yield_prediction_model.joblib")
joblib.dump(scaler_reg, "testing_it_again_reg_scaler.joblib")

print(f"\nSaved best crop selection model: {best_cls_row['Model']}")
print(f"Saved best yield prediction model: {best_reg_row['Model']}")
